...
script/style/iframe/svg/video/audio/img는 제거한다.
Markdown 이미지는 제거한다.
...
```text
1. 한글 비율 기반 간단 판정
2. 필요 시 langdetect 같은 언어 감지 라이브러리 사용
3. 실패 시 unknown
```
...
영어 번역과 한국어 번역은 서로 다른 캐시 항목이어야 한다.

### 4.7 텍스트 전처리

최종 `description`에 들어갈 텍스트는 공백 기준 토큰 문자열로 만든다.
...
한국어 형태소 분석은 필요 시 Okt 기반으로 확장할 수 있다. 단, 전처리기가 지나치게 무거워질 경우 형태소 분석은 `generate_model.py` 단계로 이동할 수 있다.
...
HTML/Markdown 정리
언어 감지
TranslateGemma 번역기 호출로 영어/한국어 설명 생성
불용어 제거와 검색용 토큰화
